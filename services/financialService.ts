...
export class FinancialService {
  ...
  public bsm(
    ...
    const vega = (S * N_prime_d1 * Math.sqrt(T)) / 100; // Scaled for 1% change in vol

    // Scale Theta and Rho to "Trader Greeks"
    theta = theta / 365; // Scaled for 1 day decay
    rho = rho / 100;     // Scaled for 1% change in rate
  ...
  public simulateOptionChain(currentPrice: number, histVolatility: number, riskFreeRate: number, existingStrikes: number[] = []): OptionChain {
  ...
  public async getOptionChain(ticker: string, currentPrice: number, histVolatility: number, riskFreeRate: number): Promise<OptionChain> {
    try {
      // 1. Fetch available expirations (or just fetch the default chain which usually contains expirations)
      const url = `/api/yahoo?ticker=${encodeURIComponent(ticker)}&type=options`;
      const response = await fetch(url);
      if (!response.ok) throw new Error('Failed to fetch options data');

      const data = await response.json();
      const result = data.optionChain?.result?.[0];
      if (!result) throw new Error('Invalid option chain data format');

      const expirations = result.expirationDates || [];
      // Limit to first 3 expirations to avoid excessive API calls/latency if we were to fetch individually,
      // but Yahoo often returns the whole chain or we need to fetch by date.
      // The default call usually returns the nearest expiration.
      // We will try to fetch the first 3 expirations.

      const chain: OptionChain = {};
      const targetExpirations = expirations.slice(0, 3); // Get first 3 expirations

      for (const expTimestamp of targetExpirations) {
        let optionsData = result;
        // If the initial result doesn't match the timestamp (it usually matches the first one), fetch specific date
        // Actually, for simplicity, let's just use the data we have if it matches, or fetch if needed.
        // Yahoo's default response contains data for the *first* expiration.

        if (expTimestamp !== result.expirationDates[0]) {
          const expUrl = `/api/yahoo?ticker=${encodeURIComponent(ticker)}&type=options&date=${expTimestamp}`;
          const expRes = await fetch(expUrl);
          if (expRes.ok) {
            const expData = await expRes.json();
            optionsData = expData.optionChain?.result?.[0];
          }
        }
        ...
        const allOptionsRaw = [...calls.map((o: any) => ({ ...o, type: OptionType.Call })), ...puts.map((o: any) => ({ ...o, type: OptionType.Put }))];
        ...
        // Filter for near-the-money strikes to keep data manageable? 
        // For now, let's take a reasonable range around current price (e.g., +/- 20%)
        const relevantOptions = allOptionsRaw.filter((o: any) => {
          return o.strike >= currentPrice * 0.8 && o.strike <= currentPrice * 1.2;
        });

        chain[daysToMaturity] = [];

        relevantOptions.forEach((opt: any) => {
          const T = daysToMaturity / 365.0; // Use 365 for actual calendar days
          const r = riskFreeRate / 100;
          const iv = opt.impliedVolatility || histVolatility; // Fallback to hist vol if IV missing

          // Recalculate Greeks using our BSM model for consistency, or use Yahoo's if available?
          // Yahoo doesn't always provide all Greeks. Let's calculate them.
          const { price, greeks, d1, d2 } = this.bsm(currentPrice, opt.strike, T, iv, r, opt.type);
          const { price: priceHistVol, greeks: greeksHistVol } = this.bsm(currentPrice, opt.strike, T, histVolatility, r, opt.type);

          chain[daysToMaturity].push({
            id: opt.contractSymbol || `${opt.strike}-${daysToMaturity}-${opt.type}`,
            strike: opt.strike,
            maturity: daysToMaturity,
            type: opt.type,
            bsmPrice: price, // Use our calculated price or opt.lastPrice? Let's use ours for consistency with Greeks, or maybe opt.lastPrice is better for "Real Data".
            // User wants "Live Market Data". Let's use opt.lastPrice if available, but we need BSM price for theoretical values.
            // The UI shows "BSM Price". Let's stick to BSM price based on IV.
            // Actually, if we use IV from market, BSM price should match Market Price closely.
            iv: iv,
            greeks: greeks,
            d1, d2,
            bsmPriceHistVol: priceHistVol,
